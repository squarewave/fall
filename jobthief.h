...
// keep_reference should be true if you want to wait on this job, and
// false otherwise. If the job is fire-and-forget, then set it to false.
...
static int _g_jt_cache_line_size = {0};
static int _g_jt_main_thread_id = 0;
static long _g_jt_thread_count = 1;
static jt_job_deque* _g_jt_job_deques = NULL;
...
__declspec( thread ) int _t_jt_worker_id;
__declspec( thread ) int _t_jt_counter;
__declspec( thread ) int _t_jt_job_count;

HANDLE _g_jt_job_semaphore;
CRITICAL_SECTION _jt_deque_section;

void _jt_push(jt_job_data* job)
{
    jt_job_deque* deque = &_g_jt_job_deques[_t_jt_worker_id];
...
{
    jt_job_data* result = NULL;
    jt_job_deque* deque = &_g_jt_job_deques[worker_id];
    ...
    if (back > front) {
        if (InterlockedCompareExchange((volatile long*)&deque->front, deque->front + 1, front) == front) {
...
{
    _t_jt_worker_id = InterlockedIncrement(&_g_jt_thread_count) - 1;
    _t_jt_counter = _t_jt_worker_id;
...
int _jt_get_cache_line_size(void)
{
    ...
    buffer = (SYSTEM_LOGICAL_PROCESSOR_INFORMATION*)malloc(result_size);
    GetLogicalProcessorInformation(buffer, (PDWORD)&result_size);
...
{
    assert(_jt_get_current_thread_id() == _g_jt_main_thread_id);
...
void _jt_worker_run_loop(void)
{
    ...
    if (job) {
        ...
        _t_jt_counter = (_t_jt_counter + 1) % _g_jt_thread_count;
        if (_t_jt_counter == _t_jt_worker_id) {
            int next_counter = (_t_jt_counter + 1) % _g_jt_thread_count;
            job = _jt_steal(next_counter);
            if (job) {
                _jt_execute_job(job);
            } else {
                WaitForSingleObjectEx(_g_jt_job_semaphore, INFINITE, FALSE);
            }
        } else {
            job = _jt_steal(_t_jt_counter);
            if (job) {
                _jt_execute_job(job);
            }
        }
...
jt_job_data* _allocate_job(void)
{
    return (jt_job_data*)calloc(_g_jt_cache_line_size, 1);
    // char* buffer_start = (char*)_t_jt_job_buffer;
    // int count = _t_jt_job_count++ & (MAX_JOBS_PER_THREAD - 1);
    // return (jt_job_data*)(buffer_start + count * _g_jt_cache_line_size);
}
...
{
    jt_job_deque* deque = &_g_jt_job_deques[_t_jt_worker_id];
    deque->jobs = (jt_job_data**)malloc(MAX_JOBS_PER_THREAD * sizeof(jt_job_data*));
    deque->front = 0;
    deque->back = 0;
}
...
void jt_init(void)
{
    _g_jt_main_thread_id = _jt_get_current_thread_id();
    _g_jt_cache_line_size = _jt_get_cache_line_size();

    int tc = _jt_get_number_of_cores();
    _g_jt_job_deques = (jt_job_deque*)calloc(tc, sizeof(jt_job_deque));
    _g_jt_job_semaphore = CreateSemaphoreEx(NULL,
                                            0, tc,
                                            NULL, NULL,
                                            SEMAPHORE_ALL_ACCESS);

    _t_jt_worker_id = 0; // the main thread gets worker ID 0
    _jt_init_worker_job_deque();
...
void jt_run_job(jt_job_data* job, int keep_reference)
{
    ...
    _jt_push(job);
    ReleaseSemaphore(_g_jt_job_semaphore, 1, NULL);
}
